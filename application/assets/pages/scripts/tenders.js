var tenders_form = function() {

    var handletenders_form = function() {

        $('#tenders_form').validate({
        ...
        $('#tenders_form input').keypress(function(e) {
            //alert('1213');
            if (e.which == 13) {
                if ($('#tenders_form').validate().form()) {
                    $('#tenders_form').submit(); //form validation success, call ajax form submit
                }
                return false;                    
            }
        });
...
$("#add").click(function(){
    ...
    ele_clone.find('td div.form-group div.input-icon i').removeClass('fa-check fa-warning');
     ele_clone.find('.quoted_rate,.negotiated_rate').removeAttr("style");
     ele_clone.find('.err_negotiated_rate').html('');
