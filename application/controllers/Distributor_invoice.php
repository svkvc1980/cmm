...
class Distributor_invoice extends Base_controller {
	...
	public function manage_dist_invoice()
	{
        # Data Array to carry the require fields to View and Model
        $data['nestedView']['heading']="Manage Distributor Invoice ";
		$data['nestedView']['pageTitle'] = 'Manage Distributor Invoice';
        $data['nestedView']['cur_page'] = 'manage_dist_invoice';
        $data['nestedView']['parent_page'] = 'manage_dist_invoice';
        ...
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Manage Distributor Invoice', 'class' => '', 'url' => '');	

        # Search Functionality
        $p_search=$this->input->post('search_dist_invoice', TRUE);
        if($p_search!='') 
        {
        ...
        $config['base_url'] = SITE_URL . 'manage_dist_invoice/';
        # Total Records
        $config['total_rows'] = $this->Distributor_invoice_m->dist_invoice_total_num_rows($search_params);
        ...
        $data['dist_invoice_results'] = $this->Distributor_invoice_m->dist_invoice_results($current_offset, $config['per_page'], $search_params);
        
        # Additional data
        $data['distributor_list'] = $this->Distributor_invoice_m->get_active_distributor();
        $data['display_results'] = 1;
        $this->load->view('dist_invoice/dist_invoice_view',$data);
    } 
    public function view_dist_invoice_details()
    {
        ...
        # Data Array to carry the require fields to View and Model
        $data['nestedView']['heading']="Invoice Details";
        $data['nestedView']['pageTitle'] = 'Invoice Details';
        ...
        # Breadcrumbs
        $data['nestedView']['breadCrumbOptions'] = array(array('label' => 'Home', 'class' => '', 'url' => SITE_URL . 'home'));
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Manage Distributor Invoice', 'class' => '', 'url' => SITE_URL.'manage_dist_invoice');  
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Invoice Details', 'class' => '', 'url' => SITE_URL.'');
       ...
       // echo $data['inv_dos'];exit;
        ...
        $this->load->view('dist_invoice/invoice_details_view',$data);                              
    }
    public function dist_invoice_entry()
    {        
        
        # Data Array to carry the require fields to View and Model
        $data['nestedView']['heading']="Distributor Invoice ";
        $data['nestedView']['pageTitle'] = 'Distributor Invoice ';
        $data['nestedView']['cur_page'] = 'dist_invoice_entry';
        $data['nestedView']['parent_page'] = 'dist_invoice_entry';

        # Load JS and CSS Files
        $data['nestedView']['js_includes'] = array();
        $data['nestedView']['js_includes'][] = '<script type="text/javascript" src="' . assets_url() . 'pages/scripts/dist_invoice_generation.js"></script>';
        $data['nestedView']['js_includes'][] = '<script type="text/javascript" src="' . assets_url() . 'pages/scripts/dist_invoice_entry.js"></script>';
        ...
        $data['schemes'] = $this->Distributor_invoice_m->get_distributor_schemes();
        $this->load->view('dist_invoice/dist_invoice_entry_view',$data);                                   
    }
        
    public function dist_invoice_generation()
    {
        ...
        # Data Array to carry the require fields to View and Model
        $data['nestedView']['heading']="Distributor Invoice Generation";
        $data['nestedView']['pageTitle'] = 'Distributor Invoice Generation';
        $data['nestedView']['cur_page'] = 'dist_invoice_generation';
        $data['nestedView']['parent_page'] = 'dist_invoice_generation';
        ...
        $data['nestedView']['breadCrumbOptions'] = array(array('label' => 'Home', 'class' => '', 'url' => SITE_URL . 'home'));
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Manage Distributor Invoice', 'class' => '', 'url' => SITE_URL.'manage_dist_invoice');  
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Invoice Entry', 'class' => '', 'url' => SITE_URL.'dist_invoice_entry');
        
        # Additional data        
        // Checking Plant has Products or not
        /*$plant_stock = $this->Common_model->get_data('plant_product',array('plant_id'=>get_plant_id()));
        if(count($plant_stock) == 0)
        {
            $this->session->set_flashdata('response','<div class="alert alert-danger alert-dismissable">
                                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>
                                                        <strong>Sorry!</strong> You dont have stock to generate invoice.So plese Check Stock </div>');       
            redirect(SITE_URL.'dist_invoice_entry');
        }*/
            //print_r($do_numbers);exit;
            $do_results = array();
            foreach ($do_numbers as $do_number)
            {
                $do_details = $this->Distributor_invoice_m->get_do_details($do_number); 
                ...
                //echo '<pre>'; print_r($do_details);exit;
                $do_products = $this->Distributor_invoice_m->get_do_products($do_details['do_number']);
            ...
            /*** VALIDATE WHETHER DO NUMBERS BELONGS TO SAME DISTRIBUTOR   START ***/
            $dos=array();        
            $dist_ids = $this->Distributor_invoice_m->get_distributor_ids($do_numbers);
            foreach ($dist_ids as $key => $value) {
                $dos[] = $value['distributor_id']; 
            ...
            if(count($u) != 1)
            {
                 $this->session->set_flashdata('response','<div class="alert alert-danger alert-dismissable">
                                                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>
                                                            <strong>Sorry!</strong> Please enter the DO Numbers Belongs to same Distributor </div>');       
                redirect(SITE_URL.'dist_invoice_entry'); 
                exit;
            }
            /*** VALIDATE WHETHER DO NUMBERS BELONGS TO SAME DISTRIBUTOR   END ***/
            ...
            $data['form_action'] = SITE_URL.'confirm_dist_invoice_generation';
            $data['flg']=1;
            ...
            if(@$this->input->post('cancel',TRUE) == 2)
            {
                ...
                print_r($data['pos_data']);exit;
                $this->load->view('dist_invoice/eidt_dist_invoice_generation_view',$data); 
            }
            else
            {
                $this->load->view('dist_invoice/dist_invoice_generation_view',$data);  
            }                                  
    }
   
    public function confirm_dist_invoice_generation()
    {
        /*foreach($this->input->post('do_product') as $dop)
        {
        ...
        $data['nestedView']['cur_page'] = 'confirm_dist_invoice_generation';
        $data['nestedView']['parent_page'] = 'confirm_dist_invoice_generation';
        ...
        $data['nestedView']['breadCrumbOptions'] = array(array('label' => 'Home', 'class' => '', 'url' => SITE_URL . 'home'));
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Manage Distributor Invoice', 'class' => '', 'url' => SITE_URL.'manage_dist_invoice');  
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Entry', 'class' => 'active', 'url' => 'dist_invoice_entry');
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Confirm Distributor Invoice', 'class' => 'active', 'url' => '');
        ...
        {    
            $schemes = $this->Distributor_invoice_m->get_schemes($this->input->post('scheme_id',TRUE));
            if(count($schemes)>0)
            {
                $fg_schemes = array();
                foreach ($schemes as $srow) {
                    $scheme_products = $this->Distributor_invoice_m->get_scheme_products($srow['fg_scheme_id'],$products_arr);
                    //echo '<pre>';print_r($scheme_products); echo $this->db->last_query().'<br>'; exit;
                    if($scheme_products)
                    {
                        foreach ($scheme_products as $sp_row) {                        
                            $scheme_gift_product = $this->Distributor_invoice_m->get_scheme_gift_product($sp_row['fgs_product_id'],$sp_row['gift_type_id']);
        ...
        //exit;
        //echo '<pre>'; print_r($products_qty_arr);print_r($products_arr);print_r($fg_schemes); exit;
        $data['flg'] = 1;
        $data['invoice_type'] = @$this->input->post('invoice_type',TRUE);
        $data['scheme_id']   = @$this->input->post('scheme_id',TRUE);
        $data['product_tot_lifting_qty'] = $products_qty_arr;
        $data['display_results'] = 0;
        $this->load->view('dist_invoice/confirm_dist_invoice_generation_view',$data);
    }    
    public function insert_dist_invoice_generation()
    {
        /*echo '<pre>';
        print_r($_POST);
        exit;*/
        //echo count($_POST['pm_id']);exit;
        if(isset($_SESSION['invoice_entry']))
        {
            echo 'allowed';
        }
        else
        ...
        $vat = get_vat_percentage($order_id);
        $invoice_number = get_distributor_invoice_number();
        $data = array(
        ...
        $inv_do_arr = array(); $total_amount = 0; $do_ids = array();
            ...
            foreach ($do_order_product as $dop_val) {
                ...
                $op_where = array('do_ob_product_id'=>$do_ob_product_id[$do_id][$order_id][$product_id]);
            ...
            foreach ($do_ids as $doID) {
                // update do Id status
                $this->Distributor_invoice_m->updateDOStatus($doID);
                //echo $this->db->last_query().'<br>';
            }
        ...
        else
        {
            ...
            unset($_SESSION['invoice_entry']);
            exit;
            redirect(SITE_URL.'print_dist_invoice_details/'.cmm_encode($invoice_id)); exit;
            $this->session->set_flashdata('response','<div class="alert alert-success alert-dismissable">
                                                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true"></button>
                                                    <strong>Success!</strong> Invoice has been generated successfully with Invoice No: '.$invoice_number.'</div>');
        }
        redirect(SITE_URL.'manage_dist_invoice');
    }    
    public function download_dist_invoice()
    {
        if($this->input->post('download_dist_invoice')!='') {
            ...
            $invoice = $this->Distributor_invoice_m->dist_invoice_details($search_params);
    ...
    public function print_dist_invoice_list()
    {
        if($this->input->post('print_dist_invoice_list')!='') {
            ...
            $data['invoice_results'] = $this->Distributor_invoice_m->dist_invoice_details($search_params);
            $data['search_data'] = $search_params;
            $this->load->view('dist_invoice/print_dist_invoice_list',$data);
