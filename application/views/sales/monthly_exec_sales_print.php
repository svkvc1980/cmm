...
<br>
 <table border="1px" align="center" width="750" cellspacing="0" cellpadding="2">
      ...
      <?php 
      if(count($executive_sale_results) > 0)
      { 
      	$sno=1;
      	$total_qty=0;
      	$total_amount=0;
      	foreach($executive_sale_results as $row) { 
      		$total_qty+=$row['qty_in_kg'];
      		$total_amount+=$row['amount'];
      		?>
      	<tr>
      		<td> <?php echo $sno++; ?></td>
      		<td> <?php echo $row['agency_name'].'['.$row['dist_code'].']'; ?></td>
    ...
    <a class="button print_element" href="<?php echo SITE_URL.'executive_monthly_sales_report';?>">Back</a>
    </div>
</body>
