...
<h3 align="center">Consolidated Unit Sales Report<span style="margin-left:50px;">From : <?php echo $from_date; ?></span><span style="margin-left:10px;">To : <?php echo $to_date; ?></span></h3>
<br>
...
<tr>   
        ...
        <?php } ?>
        <td align="center">Sachets[Qty]</td>
        <td align="center">Sales Value</td>
    </tr> 
    <?php $sno=1; 
          $grd_amount = 0; 
          $grd_pouch = 0; 
          $grd_wt = 0;
          $grd_p = array(); $unit_gt_wts = array();
          foreach ($product_results as $key =>$value)
        {   $grand_wt = 0; $grand_amount = 0; $grand_pouch = 0; $unit_tot_wts = array();?>
        ...
        <tr>
            ...
            { ?>
                ...
                {  
                    if (@$sales_details[$unit['plant_id']][$values['product_id']]!='') { $pouch = $sales_details[$unit['plant_id']][$values['product_id']]['pouch']; } else { $pouch = 0; } 
                    $pouch_count+=$pouch;

                    if (@$sales_details[$unit['plant_id']][$values['product_id']]!='') { $weight = $sales_details[$unit['plant_id']][$values['product_id']]['weight']; } else { $weight = 0; } 
                    $wt_count+=$weight;

                    $unit_tot_wts[$unit['plant_id']][]= $weight;

                    if (@$sales_details[$unit['plant_id']][$values['product_id']]!='') { $amount = $sales_details[$unit['plant_id']][$values['product_id']]['amount']; } else { $amount = 0; } 
                    $amount_count+=$amount;

                    ?>
                    <td align="right"><?php if (@$sales_details[$unit['plant_id']][$values['product_id']]!='') { echo round($sales_details[$unit['plant_id']][$values['product_id']]['pouch']); } else { echo '-'; } ?></td><?php 
                } ?>
            ...
            {  /*$carton_count = 0;
                ...
                { 
                    if (@$sales_details[$unit['plant_id']][$values['product_id']]!='') { $carton = $sales_details[$unit['plant_id']][$values['product_id']]['pouch']; } else { $carton = 0; }
                    $carton_count += $carton;
                    @$grd_p[$unit['plant_id']] += $carton;

                }*/
                $tot_wt = array_sum($unit_tot_wts[$unit['plant_id']]);
                $unit_gt_wts[$unit['plant_id']][] = $tot_wt; ?>
        ...
        <?php
         ...
         <tr><td></td> <td align="right"><b>Grand Total</b></td> 
          <?php 
            foreach ($units as $unit) { 
                $gt_wt = array_sum($unit_gt_wts[$unit['plant_id']]);
                ?>
...
<div class="row" style="text-align:center">
    ...
    <a class="button print_element" href="<?php echo SITE_URL.'consolidated_sales_view';?>">Back</a>
</div>
