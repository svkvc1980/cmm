...
<body>
    <h2 align="center">AP Cooperative Oilseeds Growers Federation Ltd</h2>
     <h4 align="center">Consolidated Leakage Report
    <?php if($from_date !='' || $to_date !='')
    { ?>
    <span style="margin-left:50px;"><?php if($from_date!='') { echo 'From : '.format_date($from_date); } ?></span><span style="margin-left:10px;"><?php if($to_date!='') { echo 'To : '.format_date($to_date); } ?></span>
    <?php } ?></h4>
    ...
    <table border="1px" align="center" width="900" cellspacing="0" cellpadding="2">
        ...
        <?php 
            if(count($leakage_report)>0)
                {  $sn=1; 
                    $gt_oil_loss=0;
                    $gt_amount_loss=0;
                    foreach($leakage_report as $key =>$value)
                    { ?>
                      <tr style="background-color:#cccfff">
                        <td colspan="9" align="left" ><b><?php echo get_plant_name_by_id($value['plant_id']); ?></b></td>
                      </tr>
                       <?php
                       $total_oil_loss=0;
                       $total_amount_loss=0;
                        ...
                        { 
                           ...
                           $loss_amount=($oil_loss*((@$latest_price_details[$row['plant_id']][$row['product_id']]['old_price'])/$row['oil_weight']));
                           $total_oil_loss+=$oil_loss;
                           $total_amount_loss+=$loss_amount;
                           $gt_oil_loss+= $oil_loss;
                           $gt_amount_loss+=$loss_amount;
                           ?>
                            <tr>
                                <td><?php echo $sn++?></td>
             ...
             <?php   } ?>
                    <tr style="background-color:#cccfff">
                        ...
                        <td align="right"><b><?php echo qty_format($gt_oil_loss); ?></b></td>
                        <td align="right"><b><?php echo price_format($gt_amount_loss); ?></b></td>
                    </tr>
         <?php   }
           else
            { ?>
    ...
    <a class="button print_element" href="<?php echo SITE_URL.'consolidated_leakage_report';?>">Back</a>
    </div>
</body>
